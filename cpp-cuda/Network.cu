...
void Network::zero_grad() const {
    for (auto const& parameter : this->parameters()) {
        ...
        parameter->grad = std::make_shared<Matrix>(num_rows, num_cols, vector<double>(num_cols*num_rows, 0.0));
    }
}

void Network::update_parameter(const double learning_rate) const {
    for (auto const& parameter : this->parameters()) {
        parameter->data = matSub(parameter->data ,parameter->grad * learning_rate);
    }
}
